...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            # 查找display_name为'小金库'的ItemType
            treasury_item_type = ItemType.objects.filter(
                display_name='小金库'
            ).first()

            if treasury_item_type:
                self.stdout.write(f'找到小金库道具类型: name="{treasury_item_type.name}", display_name="{treasury_item_type.display_name}"')

                # 检查name字段是否需要修复
                if treasury_item_type.name != 'little_treasury':
                    self.stdout.write(f'检测到错误的name字段: "{treasury_item_type.name}"，正在修复...')
                    ...
                    self.stdout.write(
                        self.style.SUCCESS('✅ 小金库道具类型的name字段已经是正确的')
                    )
